...
function CompilePanel() {
  ...
  const [testResults, setTestResults] = useState<ResultsType[]>([]);
  const [isRunning, setIsRunning] = useState(false);
  const [expectedOutput1, setExpectedOutput1] = useState<string | null>(null);
    ...
    try {
      ...
      for (let i = 0; i < compiletestCases.length; i++) {
        ...
        try {
          ...
          const { stdout, stderr, code, output } = response.data.run;
          setExpectedOutput1(stdout); // Set expected output for the test case
          setActualOutput(output); // Set actual output for the test case
          ...
          if (code !== 0 || stderr) {
            results.push({
              ...
              input: actualInput,
              expectedOutput: testCase.expectedOutput!,
              actualOutput: output || "Runtime error occurred",
              passed: false,
              expanded: true, // Auto-expand failed tests
            });
            continue;
          ...
          const normalizedExpected = normalizeOutput(expectedOutput1!);
          const passed = normalizedActual === normalizedExpected;
      ...
      {isRunning ? (
              ...
              {result.expanded && (
                <div className="p-3 bg-gray-900 text-xs">
                  ...
                  <div>
                    <div
                      className={`mb-1 ${
                        !result.passed ? "text-red-400" : "text-gray-400"
                      }`}
                    >
                      Actual Output:
                    </div>
                    <pre
                      className={`p-2 rounded whitespace-pre-wrap overflow-x-auto ${
                        result.passed
                          ? "bg-gray-800"
        ...
        <div className="flex flex-col items-center justify-center h-32 text-center text-gray-500">
          ...
          <p className="text-xs mt-2">
            Click the "Run Tests" button to execute your code against all test
            cases.
          </p>
        </div>
      )}
